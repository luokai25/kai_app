...
function renderMessages(msgs){
  ...
  c.innerHTML = msgs.map(m=>{
    ...
    const cleanText = rawText
      ...
      .trim();
    const body = m.role === 'user' ? esc(cleanText||rawText) : linkify(cleanText||rawText);
    ...
    if(m.role==='kai' && m.id && !m.meta?.error){
      ...
      feedbackRow = `<div class="fb-row" id="fb-${m.id}">
        <button class="fb-btn" onclick="sendFeedback('${m.id}',1)" title="Good">👍</button>
        <button class="fb-btn" onclick="sendFeedback('${m.id}',-1)" title="Bad">👎</button>
        ${fb===1?'<span class="fb-note" style="color:var(--good)">noted ✓</span>':fb===-1?'<span class="fb-note" style="color:var(--bad)">learning ✓</span>':''}
      </div>`;
    }
...
function promptReelGen(){
  const topic = prompt('Reel topic + options:\n\nTypes: motivational · educational · storytelling · finance · fitness · spiritual · tech\nStyles: minimal · dark-luxury · aesthetic · cinematic · corporate\n\nExamples:\n"success mindset --type=motivational --style=dark-luxury"\n"5 money habits --type=finance --scenes=6"');
  if(topic?.trim()) generateReel(topic.trim());
}
async function generateImage(prompt, style=''){
  ...
  try{
    ...
    localMsgs[localMsgs.length-1] = {role:'kai', text: r.url, meta:{ tokens:0 }};
    renderMessages(localMsgs);
  }catch(e){
...
async function generateReel(input){
  ...
  try{
    ...
    if(r.error) throw new Error(r.error);
    if(r.chat_id) currentChatId = r.chat_id;
    let reply = `🎬 **Reel: "${topic}"** (${type} · ${style})\n\n${r.script_summary||''}\n\n`;
    if(r.file_url) reply += `📄 Full reel file: ${r.file_url}\n`;
    if(r.image_urls?.length) reply += `\n${r.image_urls.join('\n')}`;
    localMsgs[localMsgs.length-1] = {role:'kai', text: reply, meta:{ tokens:r.tokens||0 }};
    renderMessages(localMsgs);
  }catch(e){
